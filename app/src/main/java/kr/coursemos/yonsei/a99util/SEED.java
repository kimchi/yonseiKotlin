...
//import kr.co.opensns.parser.PacketType;

//import kr.co.opensns.util.PropertiesReader;
...
public class SEED {
	...
	private static void SeedDecrypt(byte pbData[], // [in] encrypted data
	...
	public static byte[] AddPadding(byte[] bytes) throws Exception {
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		baos.write(bytes);
		int lastLen = baos.size() % SeedBlockSize;
		lastLen = SeedBlockSize - lastLen;
		for (int i = 0; i < lastLen; i++) {
			baos.write((byte) lastLen);
		}
		return baos.toByteArray();
	}
	public static byte[] Encrypt( String plainText, String charset,byte[] iv,byte[] key) throws Exception {
		return Encrypt(plainText.getBytes(charset),iv,key);
	}
	public static byte[] Encrypt(byte[] bytes,byte[] iv,byte[] key) throws Exception {
		byte[] pbPlain = AddPadding(bytes);
		byte[] pbCipher = new byte[pbPlain.length];

		int[] pdwRoundKey = getSeedRoundKey(key);
	...
	public static byte[] RemovePadding(byte[] decryptBytes) throws Exception {
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		int size = decryptBytes.length;
		int emptySize = 1;
		int lastValue = decryptBytes[size-1];
		if (lastValue <= SeedBlockSize) {
			for (emptySize = 1; true; emptySize++) {
				if (decryptBytes[size - emptySize] != (byte) lastValue) {
					emptySize--;
					break;
				}
			}
		...
		baos.write(decryptBytes, 0, size - emptySize);
		return baos.toByteArray();
	}
	public static String Decrypt(byte[] encryptBytes, String charset,byte[] iv,byte[] key) throws Exception {
		return new String(Decrypt(encryptBytes,iv,key), Charset.forName(charset));
	}
	public static byte[] Decrypt(byte[] encryptBytes,byte[] iv,byte[] key) throws Exception {
	...
	public static final byte[] iv = new byte[] { (byte) 0x1A, (byte) 0x8D, (byte) 0x66, (byte) 0xA7, (byte) 0x26, (byte) 0x35, (byte) 0xA8, (byte) 0x81, (byte) 0x6F, (byte) 0xBA, (byte) 0x16, (byte) 0xD9, (byte) 0xFA, (byte) 0x36, (byte) 0x25, (byte) 0x01 };
	public static final byte[] key = new byte[] { (byte) 0xE9, (byte) 0xE3, (byte) 0x4F, (byte) 0x88, (byte) 0x08, (byte) 0x17, (byte) 0x79, (byte) 0xF1, (byte) 0x8F, (byte) 0xF3, (byte) 0x94, (byte) 0x37, (byte) 0x0A, (byte) 0x89, (byte) 0xD4, (byte) 0x05 };

	public static void main(String[] args) throws Exception {
		String txt = "11111111111111111111111227122733111asfafsafs1111111adsgagdsggsd111112asfasffs27122733";
		// FCSFeIFnff5rtTGLMFEtJg==
		// Encrypt
		System.out.println(txt+"     "+txt.getBytes("utf-8").length);
		byte pbCipher[] = SEED.Encrypt(txt, "utf-8",iv,key);
		System.out.println(pbCipher.length);
//		String strenc1 = Base64Coder.encodeToString(pbCipher);
//		System.out.println(strenc1+"     "+strenc1.getBytes("utf-8").length);

		// Decrypt
		String data = SEED.Decrypt(pbCipher, "utf-8",iv,key);
		System.out.println(data+"     "+data.getBytes("utf-8").length);
		// ln3Iv38Zf6I1wfb0UFV8ZvD3GfBfFAcWslxFvBJcrDUEJJzWTipc792OsLoaEh4dnagmhjP7HGKEnaJrRa5I+w==
